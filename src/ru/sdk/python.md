# **Интеграция DV.net в ваше Python-приложение с помощью dv-net-client**

Библиотека dv-net-client предоставляет удобный способ взаимодействия с DV.net API напрямую из ваших Python‑приложений. Независимо от того, создаете ли вы веб-бэкенд, скрипт или любую другую систему на Python, этот клиент упрощает задачи вроде создания платежных запросов (счетов) и обработки обновлений статусов через вебхуки.  
Это руководство проведет вас через ключевые шаги по началу работы с dv-net-client.  
**Предварительные требования:**

* Установленный Python 3.8 или выше.
* pip (установщик пакетов Python).
* Активная учетная запись DV.net.
* Базовое понимание Python (включая asyncio при использовании асинхронного клиента).

### **Шаг 1: Установка библиотеки DV.net Client**

Сначала установите пакет с помощью pip. Откройте терминал или командную строку и выполните:  
pip install dv-net-client

Эта команда скачает и установит последнюю версию клиентской библиотеки и ее зависимости.

### **Шаг 2: Получение учетных данных DV.net API**

Для взаимодействия с DV.net API вам потребуются три значения из вашей учетной записи DV.net:

1. **API URL:** Базовый URL вашего инстанса DV.net (например, https://api.your-dv-instance.com).
2. **API Key:** Ваш публичный ключ API.
3. **API Secret:** Ваш приватный секрет API.

Сгенерировать эти данные можно в панели управления вашей учетной записью DV.net, обычно в разделе «API Keys» или «Developer».  
...
Библиотека предлагает синхронный и асинхронный клиенты. Выберите тот, что соответствует архитектуре вашего приложения.  
**Синхронный клиент:**  
Подходит для традиционных скриптов или веб-фреймворков вроде Flask/Django, если вы не используете async‑представления.
...
**Асинхронный клиент:**  
Идеален для приложений на asyncio, например FastAPI, Starlette, или асинхронных скриптов.  
...
asyncio.run(main())
```
### **Шаг 4: Создание платежного запроса (счета)**

Создание счета — распространенный сценарий. Вам нужно указать сумму, валюту и при необходимости ID заказа вашей системы.  
...
Вебхуки необходимы для получения обновлений о статусах платежей в реальном времени (например, когда счет оплачен). DV.net отправляет POST‑запросы на URL, который вы настраиваете в своей учетной записи DV.net.  
**Безопасность:** Важно проверять, что входящие запросы вебхуков действительно отправлены DV.net. Библиотека dv-net-client предоставляет для этого утилиту, использующую Webhook Secret, который вы задаете.

1. **Настройте вебхук в DV.net:**
    * Перейдите в раздел Webhooks в панели управления DV.net.
    * Установите **Payload URL** на эндпоинт вашего приложения, который будет обрабатывать эти запросы (например, https://yourdomain.com/webhooks/dvnet).
    * Создайте надежный, уникальный **Webhook Secret** и сохраните его в безопасном месте.
    * Настройте события, которые хотите получать (например, payment.completed, payment.failed).
...
```
*(В примере используется FastAPI, но функции verify_webhook_signature и WebhookMapper.map_webhook можно использовать с Flask, Django или любым другим Python‑фреймворком. Вам нужно будет адаптировать способ доступа к телу запроса и заголовкам.)*

### **Заключение**

Библиотека dv-net-client предоставляет синхронные и асинхронные интерфейсы для работы с DV.net API, а также необходимые утилиты для проверки вебхуков. Следуя этим шагам, вы сможете интегрировать криптоплатежи DV.net в свое Python‑приложение эффективно и безопасно. Не забудьте обратиться к исходному коду библиотеки или документации для подробностей обо всех доступных методах и DTO.